...
public class DocDataServiceImpl implements DocDataService {
    ...
    @Override
    public Map<String, Object> loadDocDataByCode(String DOC_DATA_CODE_) {
        String sql = "select * from KV_DOC_DATA where DOC_DATA_CODE_ = ?";
        List<Map<String, Object>> result = msJdbcTemplate.queryForList(sql, DOC_DATA_CODE_);
        if (result.size() == 1) {
            return result.get(0);
        }
        else {
    ...
    public List<Map<String, Object>> selectDocDataByIdList(List<String> DOC_DATA_ID_LIST) {
        if (DOC_DATA_ID_LIST == null || DOC_DATA_ID_LIST.isEmpty()) {
    ...
    @Override
    public int insertDocData(String DOC_DATA_ID_, String DOC_ID_, String BOOKMARK_, String DATA_TYPE_, String VALUE_, Integer ORDER_) {
        String sql = "insert into K_DOC_DATA (DOC_DATA_ID_, DOC_ID_, BOOKMARK_, DATA_TYPE_, VALUE_, ORDER_) values (?, ?, ?, ?, ?, ?)";
        return msJdbcTemplate.update(sql, DOC_DATA_ID_, DOC_ID_, BOOKMARK_, DATA_TYPE_, VALUE_, ORDER_);
    }

    @Override
    public int updateDocData(String DOC_DATA_ID_, String BOOKMARK_, String DATA_TYPE_, String VALUE_, Integer ORDER_) {
        String sql = "update K_DOC_DATA set BOOKMARK_ = ?, DATA_TYPE_ = ?, VALUE_ = ?, ORDER_ = ? where DOC_DATA_ID_ = ?";
        return msJdbcTemplate.update(sql, BOOKMARK_, DATA_TYPE_, VALUE_, ORDER_, DOC_DATA_ID_);
    }
    ...
    @Override
    public int disableDocData(String DOC_DATA_ID_, Date UPDATE_DATE_, String OPERATOR_ID_, String OPERATOR_NAME_) {
        String sql = "update K_DOC_DATA set DOC_DATA_STATUS_ = '0', UPDATE_DATE_ = ?, OPERATOR_ID_ = ?, OPERATOR_NAME_ = ? where DOC_DATA_ID_ = ?";
        return msJdbcTemplate.update(sql, UPDATE_DATE_, OPERATOR_ID_, OPERATOR_NAME_, DOC_DATA_ID_);
    }

    @Override
    public int enableDocData(String DOC_DATA_ID_, Date UPDATE_DATE_, String OPERATOR_ID_, String OPERATOR_NAME_) {
        String sql = "update K_DOC_DATA set DOC_DATA_STATUS_ = '1', UPDATE_DATE_ = ?, OPERATOR_ID_ = ?, OPERATOR_NAME_ = ? where DOC_DATA_ID_ = ?";
        return msJdbcTemplate.update(sql, UPDATE_DATE_, OPERATOR_ID_, OPERATOR_NAME_, DOC_DATA_ID_);
    }
    ...
    @Override
    public int deleteDocDataByDocIdAndBookmark(String DOC_ID_, String BOOKMARK_, Date UPDATE_DATE_, String OPERATOR_ID_, String OPERATOR_NAME_) {
